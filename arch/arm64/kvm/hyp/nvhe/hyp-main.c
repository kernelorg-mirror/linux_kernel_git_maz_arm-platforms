...
static void handle_pvm_entry_hvc(const struct kvm_vcpu *host_vcpu, struct kvm_vcpu *shadow_vcpu)
{
	...
	vcpu_set_reg(shadow_vcpu, 3, vcpu_get_reg(host_vcpu, 3));
}
...
static void handle_pvm_exit_abt(struct kvm_vcpu *host_vcpu, const struct kvm_vcpu *shadow_vcpu)
{
	host_vcpu->arch.flags = 0;
	host_vcpu->arch.fault.esr_el2 = shadow_vcpu->arch.fault.esr_el2;
	host_vcpu->arch.fault.far_el2 = shadow_vcpu->arch.fault.far_el2;
	...
	__vcpu_sys_reg(host_vcpu, SCTLR_EL1) =  __vcpu_sys_reg(shadow_vcpu, SCTLR_EL1);

	/* Sanitize. */
	host_vcpu->arch.fault.far_el2 &= FAR_MASK;
	host_vcpu->arch.ctxt.regs.pstate &= PSR_MODE_MASK;
	__vcpu_sys_reg(host_vcpu, SCTLR_EL1) &= (SCTLR_ELx_EE | SCTLR_EL1_E0E);
}
...
static struct kvm_vcpu *get_shadow_vcpu(struct kvm_vcpu *host_vcpu)
{
	...
	if (shadow_vcpu) {
		...
		shadow_vcpu->arch.host_thread_info = host_vcpu->arch.host_thread_info;
...
{
	vcpu->arch.pkvm.exit_code = exit_code;
	handle_shadow_exit(vcpu);
}
