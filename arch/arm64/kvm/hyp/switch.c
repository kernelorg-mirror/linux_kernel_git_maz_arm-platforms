					...
					bool hyp_ctxt)
{
	 ...
	 * (5) is_hyp_ctxt() now says "guest" (pstate.M==EL0 and TGE==0)
	 *
	 * If host preemption occurs, vcpu_load/put() will be very confused.
	 */

	pstate = read_sysreg_el2(SYS_SPSR);
...
int kvm_vcpu_run_vhe(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * Only sample hyp_ctxt once, as we're garanteed to exit when
	 * transitioning out of host mode.
	 */
