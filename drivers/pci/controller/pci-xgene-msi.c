...
#define NR_MSI_HWIRQ		(NR_MSI_VEC / num_possible_cpus())
...
static irq_hw_number_t compute_hwirq(u32 grp_idx, u32 msir_idx, u32 intr_idx)
{
	return ((FIELD_GET(BIT(3), grp_idx) << 7)	|
		(msir_idx << 4)				|
		intr_idx);
}

static u32 irq_to_msi_data(struct irq_data *data)
{
	return data->hwirq & GENMASK(3, 0);
}

static u32 irq_to_msi_register(struct irq_data *data)
{
	return FIELD_GET(GENMASK(6, 4), data->hwirq);
}

static u32 irq_cpu_to_frame(int cpu, struct irq_data *data)
{
	return FIELD_GET(BIT(7), data->hwirq) << 3 | cpu;
}

static u32 target_reg_offset(struct irq_data *data)
{
	int cpu;

	cpu = cpumask_first(irq_data_get_effective_affinity_mask(data));

	return (FIELD_PREP(MSI_GROUP_MASK, irq_cpu_to_frame(cpu, data)) |
		FIELD_PREP(MSI_INTR_MASK, irq_to_msi_register(data)));
}
...
static void xgene_compose_msi_msg(struct irq_data *data, struct msi_msg *msg)
{
	...
	target_addr = msi->msi_addr + target_reg_offset(data);

	msg->address_hi = upper_32_bits(target_addr);
	msg->address_lo = lower_32_bits(target_addr);
	msg->data = irq_to_msi_data(data);
}
				  ...
				  unsigned int nr_irqs, void *args)
{
	...
	hwirq = find_first_zero_bit(msi->bitmap, NR_MSI_HWIRQ);
	if (hwirq < NR_MSI_HWIRQ)
...
{
	msi->inner_domain = msi_create_parent_irq_domain(of_node_to_fwnode(node),
...
{
	xgene_msi_ctrl->bitmap = devm_bitmap_zalloc(dev, NR_MSI_HWIRQ, GFP_KERNEL);
...
static void xgene_msi_isr(struct irq_desc *desc)
{
	...
	irqp = irq_desc_get_handler_data(desc);
	grp_idx = irqp - xgene_msi->gic_irq;

	grp_pending = xgene_msi_int_read(xgene_msi, grp_idx);
	WARN_ON(grp_pending & ~GENMASK(IDX_PER_GROUP - 1, 0));

	for_each_set_bit(msir_idx, &grp_pending, IDX_PER_GROUP) {
		unsigned long msir;
		int intr_idx;

		msir = xgene_msi_ir_read(xgene_msi, grp_idx, msir_idx);
		WARN_ON(msir & ~GENMASK(IRQS_PER_IDX - 1, 0));

		for_each_set_bit(intr_idx, &msir, IRQS_PER_IDX) {
			irq_hw_number_t hwirq;
			int ret;

			hwirq = compute_hwirq(grp_idx, msir_idx, intr_idx);
...
static int xgene_msi_probe(struct platform_device *pdev)
{
	...
	for (irq_index = 0; irq_index < NR_HW_IRQS; irq_index++) {
		...
		xgene_msi->gic_irq[irq_index] = virt_msir;
	}
