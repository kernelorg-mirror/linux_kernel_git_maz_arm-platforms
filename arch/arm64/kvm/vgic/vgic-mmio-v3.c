...
static void __vgic_mmio_write_nmi(struct kvm_vcpu *vcpu, gpa_t addr,
				  ...
				  bool is_user)
{
	...
	for (i = 0; i < len * 8; i++) {
		...
		if (!is_user && irq->hw && vgic_irq_is_sgi(irq->intid) &&
		    was_nmi != irq->nmi)
				...
				unsigned int len, unsigned long val)
{
	...
	__vgic_mmio_write_nmi(vcpu, addr, len, val, false);
}

static int vgic_mmio_uaccess_write_nmi(struct kvm_vcpu *vcpu, gpa_t addr,
				       unsigned int len, unsigned long val)
{
	__vgic_mmio_write_nmi(vcpu, addr, len, val, true);
	return 0;
}
...
static const struct vgic_register_region vgic_v3_dist_registers[] = {
	...
	REGISTER_DESC_WITH_BITS_PER_IRQ_SHARED(GICD_INMIR,
		...
		NULL, vgic_mmio_uaccess_write_nmi, 1,
		VGIC_ACCESS_32bit),
...
static const struct vgic_register_region vgic_v3_rd_registers[] = {
	...
	REGISTER_DESC_WITH_LENGTH_UACCESS(SZ_64K + GICR_INMIR0,
		vgic_mmio_read_nmi, vgic_mmio_write_nmi,
		NULL, vgic_mmio_uaccess_write_nmi, 4,
		VGIC_ACCESS_32bit),
};
