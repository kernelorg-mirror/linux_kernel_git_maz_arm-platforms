...
static void aic_ipi_mask(struct irq_data *d)
{
	...
	atomic_andnot(irq_bit, &aic_vipi_mask[this_cpu]);
}
...
static void aic_ipi_unmask(struct irq_data *d)
{
	...
	/*
	 * This must complete before the atomic_read() below to avoid racing
	 * aic_ipi_send_mask(), hence we need at least acquire semantics.
	 */
	val = atomic_fetch_or_acquire(irq_bit, &aic_vipi_mask[this_cpu]);

	if (atomic_read(&aic_vipi_flag[this_cpu]) & irq_bit)
		/* Pending vIPI was unmasked, so raise a HW IPI to ourselves */
...
static void aic_ipi_send_mask(struct irq_data *d, const struct cpumask *mask)
{
	...
	for_each_cpu(cpu, mask) {
		/*
		 * This must complete before the atomic_read() below, and after
		 * any stores to normal memory; atomic_fetch_or() is fully
		 * ordered and satisfies this.
		 */
		pending = atomic_fetch_or(irq_bit, &aic_vipi_flag[cpu]);

		if (!(pending & irq_bit) && (atomic_read(&aic_vipi_mask[cpu]) & irq_bit))
	...
	if (send) {
		 ...
		 * the physical IPI. It's not entirely clear whether SMP
		 * barriers (and atomic semantics), i.e. dmb, guarantee order
		 * against Device memory accesses, so use dsb. This pairs
		 * with the mb() below.
		 */
		wmb();
...
static void aic_handle_ipi(struct pt_regs *regs)
{
	int this_cpu = smp_processor_id();
	int i;
	unsigned long mask, firing;

	aic_ic_write(aic_irqc, AIC_IPI_ACK, AIC_IPI_OTHER);

	/*
	 * Ensure that we've received and acked the IPI before we load the vIPI
	 * flags. This pairs with the wmb() above.
	 */
	mb();

	mask = atomic_read(&aic_vipi_mask[this_cpu]);
	firing = atomic_fetch_andnot(mask, &aic_vipi_flag[this_cpu]) & mask;

	/*
	 * atomic_fetch_andnot() above is fully ordered, and pairs with the
	 * atomic_fetch_or() in aic_ipi_send_mask() to ensure ordering
	 * between flag exchanges and the IPI handler.
	 */
...
static int aic_init_cpu(unsigned int cpu)
{
	 ...
	 * If we ever end up with a mismatch h	ere, we will have to introduce
...
static struct gic_kvm_info vgic_info = {
	.type	= APL_VGIC_V3,
};
