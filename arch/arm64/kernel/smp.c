...
#define irq_to_ipi(irq)		((irq - ipi_irq_base) % nr_ipi_cpus)

static bool percpu_ipi_descs;
static int nr_ipi_cpus;
...
static void ipi_setup_ppi(int ipi)
{
	...
	if (ipi_should_be_nmi(irq)) {
		err = request_percpu_nmi(irq, ipi_handler, "IPI", &cpu_number);
		WARN(err, "Could not request IRQ %d as NMI, err=%d\n", irq, err);
	} else {
		err = request_percpu_irq(irq, ipi_handler, "IPI", &cpu_number);
...
static void ipi_setup_lpi(int ipi, int ncpus)
{
	for (int cpu = 0; cpu < ncpus; cpu++) {
		...
		irq = ipi_irq_base + ipi + cpu;

		err = request_irq(irq, ipi_handler, IRQF_NO_AUTOEN, "IPI", &cpu_number);
