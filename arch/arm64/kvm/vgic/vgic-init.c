...
static int vgic_allocate_ppis_locked(struct kvm_vcpu *vcpu)
{
	 ...
	 */
	for (int i = 0; i < VGIC_NR_PRIVATE_IRQS; i++) {
...
static int vgic_allocate_ppis(struct kvm_vcpu *vcpu)
{
	int ret;

	mutex_lock(&vcpu->kvm->arch.config_lock);
	ret = vgic_allocate_ppis_locked(vcpu);
