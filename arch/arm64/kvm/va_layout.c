			     ...
			     __le32 *origptr, __le32 *updptr, int nr_inst)
{
	...
	/*
	 * If we're running at EL1 and that we don't have hVHE, then
	 * SCTLR_EL2.SPAN means nothing to us, and we don't need to set
	 * PSTATE.PAN to anything useful.
	 */
	if (!is_kernel_in_hyp_mode() && !cpus_have_cap(ARM64_KVM_HVHE))
		return;

	/*
	 * Leap of faith: at this point, we're sure we're running VHE, and
	 * we assume that FEAT_PAN is implemented, no ifs, no buts. If KVM
	 * explodes at runtime, call your favourite HW support.
	 *
