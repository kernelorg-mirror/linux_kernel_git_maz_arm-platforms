			 ...
			 const struct sys_reg_desc *r)
{
	...
	if (nested_virt_in_use(vcpu) && forward_traps(vcpu, HCR_AT))
		return false;
			...
			const struct sys_reg_desc *r)
{
	...
	if (nested_virt_in_use(vcpu) && forward_nv_traps(vcpu))
		return false;
			    ...
			    const struct sys_reg_desc *r)
{
	...
	if (nested_virt_in_use(vcpu) && forward_traps(vcpu, HCR_TTLB))
		return false;
