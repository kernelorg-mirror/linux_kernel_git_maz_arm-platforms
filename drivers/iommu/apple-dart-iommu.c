 ...
 * @num_devices: reference count for devices attached to this stream
 ...
 * are only ever appened and never removed.
 *
 * @num_streams: number of streams attached
 * @streams: array of structs to identify attached streams
 */
...
static void apple_dart_hw_disable_translation(struct apple_dart *dart, u16 sid)
{
					...
					u32 command)
{
	...
	if (ret) {
		dev_err(dart->dev,
			"Timeout while waiting for busy flag to be cleared after issuing command %08x for streams %x\n",
			command, sid_bitmap);
		return ret;
	}
...
static int apple_dart_hw_reset(struct apple_dart *dart)
{
	...
	for (sid = 0; sid < DART_MAX_STREAMS; ++sid) {
		apple_dart_hw_disable_translation(dart, sid);
...
static int apple_dart_finalize_domain(struct iommu_domain *domain)
{
	...
	/*
	 * if we already have pgtbl_ops here this function has alrady been
	 * previously called for this domain and we don't need to do anything
	 */
...
static irqreturn_t apple_dart_irq(int irq, void *dev)
{
	...
	u32 addr = readl(dart->regs + DART_ERROR_ADDR_LO);
	u8 domain_idx =
	...
	if (__ratelimit(&rs)) {
		dev_err(dart->dev,
			"Apple DART translation fault: error status %08x [stream:%d code:%x (%s)] at address 0x%08x",
